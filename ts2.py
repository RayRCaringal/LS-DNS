...
def run():
    ...
    EOF = 1
    while EOF != 0:
        print("[TS2]: Waiting on Client")
        reply = clientsocket.recv(1024)
        hostName = reply.decode('utf-8').rstrip().lower()
        if hostName == "eof":
            print ("[TS2]: Exiting a client at {}".format(addr))
            return
        print ("[TS2]: Request from a client for hostname " + hostName)
        if hostName in table:
            print ("[TS2]: Hostname " + hostName + " found sending IP Address " + table.get(hostName).ip + " to Client")
            clientsocket.send((table.get(hostName).ip).encode('utf-8'))
        else:
            print ("[TS2]: Hostname " + hostName + " not found")

    
#Creates Table 
table = {}
path = os.path.dirname(os.path.realpath('__file__')) + '/PROJ2-DNSTS2.txt'
...
#Listen forever 
msg = "[TS2] Connected to Top-Level DNS2"
while True:
    ss.listen(5)
